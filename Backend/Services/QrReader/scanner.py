...
def get_hidraw_id(scrip_cmd, name_device):
    output = subprocess.getoutput(scrip_cmd)
    if len(output) > 0:
        outputs = output.split("\n")
        for out_str in outputs:
            tmp_devices = out_str.split(" connect_device ")
            if tmp_devices[1].find(name_device) != -1:
                return "/dev/{}".format(tmp_devices[0])
    return None


def connect_usb_device_by_pid_vid(pid, vid):
    dev = usb.core.find(idVendor=vid, idProduct=pid)

    dev.attach_kernel_driver(0)


def disconnect_usb_device_by_pid_vid(pid, vid):
    dev = usb.core.find(idVendor=vid, idProduct=pid)
    # unbind interface 0
...
if __name__ == "__main__":
    connect_usb_device_by_pid_vid(0x1000, 0xE851)
